# Nestling — iOS parent viewer

Reads a family's own Nestling server and shows a child's screen time. It does
...
| xcodegen | brew | regenerates `Nestling.xcodeproj` from `project.yml` |
...
## Build, test, install
    ...
    xcodebuild -project Nestling.xcodeproj -scheme Nestling \
    ...
    xcrun devicectl device install app --device <id> \
      ~/Library/Developer/Xcode/DerivedData/Nestling-*/Build/Products/Debug-iphoneos/Nestling.app
    xcrun devicectl device process launch --device <id> ch.jorisda.nestling.viewer
...
tapping (absent from release builds):
    ...
    SIMCTL_CHILD_NESTLING_SERVER=https://nestling.example.ch \
    SIMCTL_CHILD_NESTLING_EMAIL=… SIMCTL_CHILD_NESTLING_PASSWORD=… \
    SIMCTL_CHILD_NESTLING_AUTOLOGIN=1 SIMCTL_CHILD_NESTLING_OPEN_FIRST_CHILD=1 \
    xcrun simctl launch <simulator-id> ch.jorisda.nestling.viewer
